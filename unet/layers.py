...
class ConvDouble(nn.Module):
    ...
    '''
    def __init__(self, in_channels, kernel_size, endec, dropout, stride, padding, conv3d, complex, first=False):
        super(ConvDouble, self).__init__()
        if endec:
            if first:
                out_channels = complex
            else:
...
class EncoderBlock(nn.Module):
    ...
    '''
    def __init__(self, in_channels, conv_kernel_size, pool_kernel_size, dropout, conv_stride, conv_padding, conv3d, complex, first):
        super(EncoderBlock, self).__init__()
        self.doubleConv = ConvDouble(in_channels, conv_kernel_size, True, dropout, conv_stride, conv_padding, conv3d, complex, first=first)
        if conv3d:
...
class DecoderBlock(nn.Module):
    ...
    '''
    def __init__(self, in_channels, conv_kernel_size, up_kernel_size, dropout, conv_stride, conv_padding, complex, conv3d):
        super(DecoderBlock, self).__init__()
        out_channels = int(in_channels // 2)
        self.doubleConv = ConvDouble(in_channels, conv_kernel_size, False, dropout, conv_stride, conv_padding, complex, conv3d)
        if conv3d:
...
class EncoderUNet(nn.Module):
    ...
    '''
    def __init__(self, in_channels, conv_kernel_size, pool_kernel_size, dropout, conv_stride, conv_padding, conv3d, size, complex):
        super(EncoderUNet, self).__init__()
        self.blocks = nn.ModuleList()
        self.blocks.append(EncoderBlock(in_channels, conv_kernel_size, pool_kernel_size, dropout, conv_stride, conv_padding, conv3d, complex, first=True))
        in_channels = complex
        for i in range(size-1):
            self.blocks.append(EncoderBlock(in_channels, conv_kernel_size, pool_kernel_size, dropout, conv_stride, conv_padding, conv3d, complex, first=False))
            in_channels *= 2
...
class DecoderUNet(nn.Module):
    ...
    '''
    def __init__(self, in_channels, conv_kernel_size, up_kernel_size, dropout, conv_stride, conv_padding, conv3d, size, complex):
        super(DecoderUNet, self).__init__()
        self.blocks = nn.ModuleList()
        for i in range(size):
            self.blocks.append(DecoderBlock(in_channels, conv_kernel_size, up_kernel_size, dropout, conv_stride, conv_padding, complex, conv3d))
